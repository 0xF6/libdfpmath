namespace invocative.mathm
{
    ...
    public static partial class Mathm
    {
        ...
        public static decimal Sqrt(decimal s)
        {
            ...
            x = (decimal)Math.Sqrt(ToDouble(s));
        ...
        {
            Debug.Assert(y >= 0 && Truncate(y) == y, "Only non-negative, integer powers supported.");
            if (y < 0) throw new ArgumentOutOfRangeException(nameof(y), "Negative exponents not supported!");
            if (Truncate(y) != y) throw new ArgumentException("Exponent must be an integer!", nameof(y));
        ...
        public static decimal Log10(decimal d)
        {
            ...
            var parts = GetBits(d / 1.000000000000000000000000000000000m);
        ...
        public static decimal Factorial(decimal n)
        {
            if (n < 0) 
                ...
                throw new ArgumentException("Values less than zero are not supported!", nameof(n));
            if (Truncate(n) != n) throw new ArgumentException("Fractional values are not supported!", nameof(n));
        ...
        {
            if (places < 0) 
                throw new ArgumentOutOfRangeException("places", "Places must be greater than or equal to 0.");
